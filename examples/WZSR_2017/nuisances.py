...
nuisances['electronpt'] = {
    ...
    'mapUp': 'ElepTup',
    'mapDown': 'ElepTdo',
    'samples': dict((skey, ['1', '1']) for skey in mcALL), # FIXME if skey not in ["WW"]
...
nuisances['muonpt'] = {
    'name': 'scale_m_2017_UL',
    'kind': 'suffix',
    'type': 'shape',
    'mapUp': 'MupTup',
    'mapDown': 'MupTdo',
    'samples': dict((skey, ['1', '1']) for skey in mcALL if skey not in ["ZZ"]),
...
nuisances['JER'] = {
                ...
                'mapUp': 'JERup',
                'mapDown': 'JERdo',
                'samples': dict((skey, ['1','1']) for skey in mcALL if skey not in ['tVx']), # FIXME if skey not in ["tVx", "WW", "ZZ"]),
                'folderUp' : makeMCDirectory('JERup_suffix'),
                'folderDown' : makeMCDirectory('JERdo_suffix'),
}

# ------------------- JES
# ----- from Susan's cfg # FIXME jes was commented........
...
nuisances['PU']  = {
                ...
                'samples'  : {
                    s : ['(puWeightUp/puWeight)',
                         '(puWeightDown/puWeight)'] for s in mcALL}, 
}
...
nuisances['PS_ISR']  = {
    ...
    'kind': 'weight',
    'type': 'shape',
    'samples': dict((skey, ['PSWeight[2]', 'PSWeight[0]']) for skey in mcALL),
}
...
nuisances['PS_FSR']  = {
    ...
    'kind': 'weight',
    'type': 'shape',
    'samples': dict((skey, ['PSWeight[3]', 'PSWeight[1]']) for skey in mcALL),
}
...
nuisances['UE']  = {
                ...
                'skipCMS' : 1,
                'type': 'lnN',
                'samples': dict((skey, '1.015') for skey in mcALL)# if skey not in ['WW','WW']), 
}
